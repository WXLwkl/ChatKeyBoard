    ...
    if (fileName.length > 0) {
        ...
        if (_playingFileName && [fileName isEqualToString:_playingFileName]) {//上次播放的录音
            ...
            /*
             ...
             AVAudioPlayer *pl = [[AVAudioPlayer alloc] initWithContentsOfURL:[NSURL fileURLWithPath:[VoiceCommonHelper getPathByFileName:fileName ofType:@"wav"]] error:nil];
             */
...
+ (id)shareInstance {
    ...
    dispatch_once(&onceToken, ^{
        instance = [[AudioPlayerHelper alloc] init];
    });
    return instance;
}

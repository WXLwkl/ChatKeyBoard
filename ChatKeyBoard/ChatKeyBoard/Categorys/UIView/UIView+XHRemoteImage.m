...
- (void)setMessageAvatarType:(MessageAvatarType)messageAvatarType {
...
- (MessageAvatarType)messageAvatarType {
    return (MessageAvatarType)([objc_getAssociatedObject(self, &kXHMessageAvatarTypeKey) integerValue]);
}
...
- (void)loadWithCompletionBlock:(void(^)(UIImage *image, NSURL *url, NSError *error))handler {
    ...
    dispatch_async(self.cachingQueue, ^{
        ...
        if (weakSelf.messageAvatarType != MessageAvatarTypeNormal) {
            cacheImage = [MessageAvatarFactory avatarImageNamed:cacheImage messageAvatarType:weakSelf.messageAvatarType];
        }
        if (cacheImage) {
...
- (UIImage *)didFinishDownloadWithData:(NSData *)data forURL:(NSURL *)url error:(NSError *)error {
    ...
    if (self.messageAvatarType != MessageAvatarTypeNormal) {
        image = [MessageAvatarFactory avatarImageNamed:image messageAvatarType:self.messageAvatarType];
    }
    if([url isEqual:self.url]) {
        if(error) {
